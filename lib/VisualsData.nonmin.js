!function(modules) {
            ...
            !function(data) {
                ...
                var DefaultSQExprVisitorWithTraversal = function() {
                    ...
                    return DefaultSQExprVisitorWithTraversal.prototype.visitEntity = function(expr) {
                        ...
                        for (var values = expr.values, i = 0, len = values.length; i < len; i++) for (var valueTuple = values[i], j = 0, jlen = valueTuple.length; j < jlen; j++) valueTuple[j].accept(this);
        ...
        !function(powerbi) {
            ...
            var EnumExtensions = jsCommon.EnumExtensions, ValueType = function() {
                ...
                return ValueType.fromDescriptor = function(descriptor) {
                    ...
                    return !!(this === other || this.primitiveType === otherPrimitiveType || otherPrimitiveType === PrimitiveType.Null || this.numeric && other.numeric);
            ...
            !function(data) {
                ...
                !function(DataViewTransform) {
                    ...
                    function transformDataView(prototype, transformContext, selectsToInclude) {
                        ...
                        transformed = data.DataViewCategoricalProjectionOrder.apply(transformed, transformContext.applicableRoleMappings, transformContext.dataRoles, projectionOrdering, selectsToInclude), 
                    ...
                    function applyTransformsToColumns(prototypeColumns, selects, rewrites) {
                        ...
                        for (var columns = inherit(prototypeColumns), i = 0, len = prototypeColumns.length; i < len; i++) {
                    ...
                    function evaluateAggregate(evalContext, selects, type, source) {
                        ...
                        if (select) return data.DataViewObjectEvaluator.evaluateValue(evalContext, select.expr, type);
                    }
                    ...
                    function applyRewritesToTable(prototype, columnRewrites, projectionOrdering) {
                        ...
                        for (var columnsClone = columns.slice(0), keys = Object.keys(newToOldPositions), i = 0, len = keys.length; i < len; i++) {
                        ...
                        var rows = powerbi.Prototype.overrideArray(table.rows, function(row) {
                            for (var newRow = [], i = 0, len = keys.length; i < len; ++i) newRow[i] = row[newToOldPositions[keys[i]]];
                            return newRow;
                        });
                    ...
                    function transformObjects(dataView, targetDataViewKinds, objectDescriptors, objectDefinitions, selectTransforms, colorAllocatorFactory) {
                        if (objectDescriptors) {
                            ...
                            if (metadataOnce) for (var i = 0, len = metadataOnce.length; i < len; i++) {
                                var metadataOnceObject = metadataOnce[i], objectDefns = metadataOnceObject.objects;
                            ...
                            if (metadataObjects) for (var i = 0, len = metadataObjects.length; i < len; i++) {
                    ...
                    function mergeObjects(targetObjects, sourceObjects, selector) {
                        if (!_.isEmpty(sourceObjects)) for (var objectName in sourceObjects) {
                            var sourceObject = sourceObjects[objectName];
                            if (selector && void 0 !== selector.id) {
                                var targetObject = targetObjects[objectName];
                                targetObject || (targetObject = targetObjects[objectName] = {});
                                var instances = targetObject.$instances;
                                instances || (instances = targetObject.$instances = {}), instances[selector.id] = sourceObject;
                            } else targetObjects[objectName] = sourceObject;
                        }
                    }
                    function evaluateMetadataObjects(dataView, selectTransforms, objectDescriptors, selector, objectDefns, dataObjects, colorAllocatorFactory) {
                        ...
                        if (objects) {
                            ...
                            metadataObjects || (metadataObjects = dataView.metadata.objects = {}), mergeObjects(metadataObjects, objects, selector);
                    ...
                    function populateColorAllocatorCache(dataView, selectTransforms, objectDefns, colorAllocatorFactory) {
                        for (var cache = data.createColorAllocatorCache(), staticEvalContext = data.createStaticEvalContext(), i = 0, len = objectDefns.length; i < len; i++) {
                    ...
                    function getIdentityExprsForRolesOnTable(roles, dataViewTable) {
                        ...
                        if (allColumnsBelongToSelectorRole && exprs) return exprs;
                    }
                    ...
                    function isUniqueDataSelector(dataSelectors, newSelector) {
                        return !!_.isEmpty(dataSelectors) || !_.any(dataSelectors, function(dataSelector) {
                                ...
                                if (identities) {
                                    for (var i = 0, len = identities.length; i < len; i++) {
                                        ...
                                        if (containsWildcard || data.Selector.matchesData(selector, [ identity ])) {
                                            ...
                                            if (objects) {
                                                var columnObjects = targetColumn.column.objects;
                                                columnObjects || (columnObjects = targetColumn.column.objects = [], targetColumn.column.objects.length = len), 
                                                columnObjects[i] || (columnObjects[i] = {}), mergeObjects(columnObjects[i], objects, selector);
                                            }
                    ...
                    function evaluateDataRepetitionCategoricalValueGrouping(dataViewCategorical, selectTransforms, objectDescriptors, selector, rules, containsWildcard, objectDefns, colorAllocatorCache) {
                        ...
                        if (dataViewCategoricalValues && dataViewCategoricalValues.identityFields && data.Selector.matchesKeys(selector, [ dataViewCategoricalValues.identityFields ])) {
                            ...
                            if (valuesGrouped) {
                                for (var foundMatch, evalContext = data.createCategoricalEvalContext(colorAllocatorCache, dataViewCategorical, selectTransforms), i = 0, len = valuesGrouped.length; i < len; i++) {
                                    ...
                                    if (containsWildcard || data.Selector.matchesData(selector, [ valueGroup.identity ])) {
                                        ...
                                        if (objects) if (selectorMetadata) for (var j = 0, jlen = valuesInGroup.length; j < jlen; j++) {
                                            ...
                                            if (valueSource.queryName === selectorMetadata) {
                                                ...
                                                mergeObjects(valueColumnObjects, objects, selector), valueColumn.source = valueSourceOverwrite, 
                                                foundMatch = !0;
                                                break;
                                            }
                                        } else {
                                            var valueGroupObjects = valueGroup.objects;
                                            valueGroupObjects || (valueGroupObjects = valueGroup.objects = {}), mergeObjects(valueGroupObjects, objects, selector), 
                    ...
                    function evaluateDataRepetitionTableRows(evalContext, columns, rows, identities, identityFields, objectDescriptors, selector, rules, containsWildcard, objectDefns) {
                        if (!_.isEmpty(identities) && !_.isEmpty(identityFields) && selector.metadata && data.Selector.matchesKeys(selector, [ identityFields ])) {
                            ...
                            if (!(colIdx < 0)) {
                                for (var inheritedRows, colLen = columns.length, rowIdx = 0, rowLen = identities.length; rowIdx < rowLen; rowIdx++) {
                    ...
                    function evaluateMetadataRepetition(dataView, selectTransforms, objectDescriptors, selector, objectDefns, colorAllocatorCache) {
                        for (var columns = dataView.metadata.columns, metadataId = selector.metadata, evalContext = data.createStaticEvalContext(colorAllocatorCache, dataView, selectTransforms), i = 0, len = columns.length; i < len; i++) {
                            var column = columns[i];
                            if (column.queryName === metadataId) {
                                var objects = data.DataViewObjectEvaluationUtils.evaluateDataViewObjects(evalContext, objectDescriptors, objectDefns), columnObjects = column.objects;
                                columnObjects || (columnObjects = column.objects = {}), mergeObjects(columnObjects, objects, selector);
                            }
                        }
                    }
                    ...
                    function findSelectorsForRuleInput(dataView, selectorRoles) {
                        ...
                        if (dataViewCategorical) {
                            ...
                            if (categories) {
                                for (var categoryRoles, selectorsForRuleInput = [], categoryIndex = 0, categoryCount = categories.length; categoryIndex < categoryCount; categoryIndex++) {
                    ...
                    function findFirstQueryNameForExpr(selectTransforms, expr) {
                        ...
                        if (selectTransforms) for (var i = 0, len = selectTransforms.length; i < len; i++) {
                        ...
                        if (categorical && !_.isEmpty(dataViewMappings)) {
                            ...
                            if (categories && 1 === categories.length) {
                                ...
                                if (!_.isEmpty(values) && !values.grouped().some(function(vg) {
                                    return !!vg.identity;
                                })) for (var categoryRoles = categories[0].source.roles, i = 0, len = dataViewMappings.length; i < len; i++) {
                    ...
                    DataViewTransform.mergeObjects = mergeObjects, DataViewTransform.createValueColumns = createValueColumns, 
            ...
            !function(data) {
                var ArrayExtensions = jsCommon.ArrayExtensions, SQExprRewriter = function() {
                    ...
                    return SQExprRewriter.prototype.visitColumnRef = function(expr) {
                        ...
                        for (var rewrittenValues, origArgs = orig.args, rewrittenArgs = this.rewriteAll(origArgs), origValues = orig.values, i = 0, len = origValues.length; i < len; i++) {
            ...
            !function(data) {
                ...
                var SQExprEqualityVisitor = function() {
                    ...
                    return SQExprEqualityVisitor.run = function(x, y, ignoreCase) {
                        return x = x || null, y = y || null, x === y || !x == !y && (ignoreCase ? x.accept(SQExprEqualityVisitor.ignoreCaseInstance, y) : x.accept(SQExprEqualityVisitor.instance, y));
                        ...
                        return comparand instanceof SQConstantExpr && expr.type === comparand.type && (expr.type.text && this.ignoreCase ? StringExtensions.equalIgnoreCase(expr.valueEncoded, comparand.valueEncoded) : expr.valueEncoded === comparand.valueEncoded);
                        ...
                        return !!this.equals(stop1.color, stop2.color) && (stop1.value ? this.equals(stop1.value, stop2.value) : stop1.value === stop2.value);
                        ...
                        for (var i = 0; i < len; i++) if (!this.equals(x[i], y[i])) return !1;
                        return !0;
                    ...
                    return __extends(SQExprValidationVisitor, _super), SQExprValidationVisitor.prototype.visitIn = function(expr) {
                        for (var inExpr = _super.prototype.visitIn.call(this, expr), args = inExpr.args, values = inExpr.values, _i = 0, values_1 = values; _i < values_1.length; _i++) for (var valueTuple = values_1[_i], i = 0, len = valueTuple.length; i < len; ++i) this.validateCompatibleType(args[i], valueTuple[i]);
                        return inExpr;
            ...
            !function(data) {
                ...
                var CategoricalDataViewBuilder = function() {
                    ...
                    return CategoricalDataViewBuilder.prototype.withCategory = function(options) {
                        ...
                        if (this.hasDynamicSeries()) for (var seriesIndex = 0; seriesIndex < this.dynamicSeriesMetadata.values.length; seriesIndex++) for (var seriesMeasures = this.dynamicSeriesValues[seriesIndex], measureIndex = 0, measuresLen = this.dynamicMeasureColumns.length; measureIndex < measuresLen; measureIndex++) {
    ...
    385: function(module, exports, __webpack_require__) {
        ...
        window.debug = window.debug || {}, window.InJs = window.InJs || {}, __webpack_require__(386);
    },
    386: function(module, exports, __webpack_require__) {
        window.jsCommon, window.powerbi, window.powerbitests, window.InJs, window.debug, 
        window.jasmine, window.Microsoft;
        __webpack_require__(387), __webpack_require__(73), __webpack_require__(388), __webpack_require__(389), 
        __webpack_require__(390), __webpack_require__(391), __webpack_require__(392), __webpack_require__(393), 
        __webpack_require__(74), __webpack_require__(394), __webpack_require__(395), __webpack_require__(396), 
        __webpack_require__(397), __webpack_require__(75), __webpack_require__(398), __webpack_require__(399), 
        ...
        __webpack_require__(428), __webpack_require__(76), __webpack_require__(429), __webpack_require__(430), 
        __webpack_require__(431), __webpack_require__(432), __webpack_require__(433), __webpack_require__(434), 
        __webpack_require__(77), __webpack_require__(435), __webpack_require__(436), __webpack_require__(437), 
        __webpack_require__(438), __webpack_require__(439), __webpack_require__(440), __webpack_require__(441), 
        __webpack_require__(442), __webpack_require__(443), __webpack_require__(444), __webpack_require__(445), 
        __webpack_require__(446), __webpack_require__(79), __webpack_require__(447), __webpack_require__(448), 
        __webpack_require__(449), __webpack_require__(78), __webpack_require__(450), __webpack_require__(451), 
        __webpack_require__(452), __webpack_require__(80), __webpack_require__(453), __webpack_require__(454), 
        __webpack_require__(455), __webpack_require__(456), __webpack_require__(457), __webpack_require__(458), 
        __webpack_require__(81), __webpack_require__(459), __webpack_require__(460), __webpack_require__(461), 
        __webpack_require__(82), __webpack_require__(462), __webpack_require__(463);
    },
            ...
            !function(data_1) {
                ...
                !function(Selector) {
                    ...
                    function equalsData(x, y) {
                        ...
                        return selector1.expr && selector2.expr ? powerbi.DataViewScopeIdentity.equals(selector1, selector2) : selector1.exprs && selector2.exprs ? data_1.DataViewScopeWildcard.equals(selector1, selector2) : !(!selector1.roles || !selector2.roles) && data_1.DataViewRoleWildcard.equals(selector1, selector2);
                    }
        ...
        !function(powerbi) {
            var DateTimeSequence = function() {
                ...
                return DateTimeSequence.prototype.add = function(date) {
                    ...
                    for (var x = this.min; min < x; ) x = DateTimeSequence.addInterval(x, -this.interval, this.unit), 
                    this.sequence.splice(0, 0, x);
                    for (this.min = x, x = this.max; x < max; ) x = DateTimeSequence.addInterval(x, this.interval, this.unit), 
                    ...
                    if (0 !== interval) {
                        ...
                        for (dexp = 0; dexp < 3; dexp++) {
                            for (var e = exp + dexp, power = powerbi.Double.pow10(e), roundMin = powerbi.Double.floorToPrecision(range.min, power), roundMax = powerbi.Double.ceilToPrecision(range.max, power), stepsCount = steps.length, stepPower = powerbi.Double.pow10(e - 1), i = 0; i < stepsCount; i++) {
                                ...
                                count = powerbi.Double.ceilWithPrecision(roundRange.getSize() / step), count <= expectedCount || 2 === dexp && i === stepsCount - 1 || 1 === expectedCount && 2 === count && (step > range.getSize() || range.min < 0 && range.max > 0 && 2 * step >= range.getSize())) {
            ...
            !function(visuals) {
                ...
                !function(valueFormatter) {
                    ...
                    function shouldUseNumericDisplayUnits(options) {
                        ...
                        if ("number" == typeof value || "number" == typeof value2) return !0;
                    }
            ...
            !function(data) {
                ...
                var DataViewCategoricalReader = function() {
                    function DataViewCategoricalReader(dataView) {
                        ...
                        if (categorical && (values = categorical.values), this.hasAnyValidValues = !1, null != values) {
                            ...
                            if (grouped.length > 0) {
                                ...
                                for (var valueRoleIndexMapping = {}, firstGroupValues = grouped[0].values, valueIndex = 0, valueCount = firstGroupValues.length; valueIndex < valueCount; valueIndex++) {
                    ...
                    return DataViewCategoricalReader.prototype.hasCategories = function() {
                        ...
                        if (this.hasValidCategories) return this.getCategoryFromRole(roleName);
                        ...
                        if (this.hasValidCategories) return this.categories.length > 1;
                        ...
                        return !!this.hasValues(roleName) && !_.isEmpty(this.grouped[0].values[this.valueRoleIndexMapping[roleName][0]].highlights);
                    }, DataViewCategoricalReader.prototype.getValue = function(roleName, categoryIndex, seriesIndex) {
                        if (void 0 === seriesIndex && (seriesIndex = 0), this.hasValues(roleName)) return this.dataHasDynamicSeries ? this.getValueInternal(roleName, categoryIndex, seriesIndex, 0, !1) : this.getValueInternal(roleName, categoryIndex, 0, seriesIndex, !1);
                    }, DataViewCategoricalReader.prototype.getHighlight = function(roleName, categoryIndex, seriesIndex) {
                        if (void 0 === seriesIndex && (seriesIndex = 0), this.hasValues(roleName)) return this.dataHasDynamicSeries ? this.getValueInternal(roleName, categoryIndex, seriesIndex, 0, !0) : this.getValueInternal(roleName, categoryIndex, 0, seriesIndex, !0);
                        ...
                        if (void 0 === seriesIndex && (seriesIndex = 0), this.hasValues(roleName)) {
                            for (var valuesInRole = [], roleValueIndex = 0, roleValueCount = this.valueRoleIndexMapping[roleName].length; roleValueIndex < roleValueCount; roleValueIndex++) valuesInRole.push(this.getValueInternal(roleName, categoryIndex, seriesIndex, roleValueIndex, !1));
                            return valuesInRole;
                        }
                    }, DataViewCategoricalReader.prototype.getAllHighlightsForRole = function(roleName, categoryIndex, seriesIndex) {
                        if (void 0 === seriesIndex && (seriesIndex = 0), this.hasValues(roleName)) {
                            for (var valuesInRole = [], roleValueIndex = 0, roleValueCount = this.valueRoleIndexMapping[roleName].length; roleValueIndex < roleValueCount; roleValueIndex++) valuesInRole.push(this.getValueInternal(roleName, categoryIndex, seriesIndex, roleValueIndex, !0));
                            return valuesInRole;
                        }
                        ...
                        if (this.hasValues(roleName)) return this.grouped[0].values[this.valueRoleIndexMapping[roleName][0]].source.queryName;
                    }, DataViewCategoricalReader.prototype.getValueColumn = function(roleName, seriesIndex) {
                        if (void 0 === seriesIndex && (seriesIndex = 0), this.hasValues(roleName)) return this.dataHasDynamicSeries ? this.grouped[seriesIndex].values[this.valueRoleIndexMapping[roleName][0]] : this.grouped[0].values[this.valueRoleIndexMapping[roleName][seriesIndex]];
                        ...
                        if (void 0 === seriesIndex && (seriesIndex = 0), this.hasValues(roleName)) {
                            for (var metadata = [], roleValueIndex = 0, roleValueCount = this.valueRoleIndexMapping[roleName].length; roleValueIndex < roleValueCount; roleValueIndex++) {
                        ...
                        if (this.hasAnyValidValues) return this.grouped[seriesIndex].objects;
                    }, DataViewCategoricalReader.prototype.getSeriesValueColumns = function() {
                        if (this.hasAnyValidValues) return this.dataView.categorical.values;
                    }, DataViewCategoricalReader.prototype.getSeriesValueColumnGroup = function(seriesIndex) {
                        if (this.hasAnyValidValues) return this.grouped[seriesIndex];
                    }, DataViewCategoricalReader.prototype.getSeriesMetadataColumn = function() {
                        if (this.hasAnyValidValues) return this.dataView.categorical.values.source;
                    }, DataViewCategoricalReader.prototype.getSeriesColumnIdentityFields = function() {
                        if (this.hasAnyValidValues) return this.dataView.categorical.values.identityFields;
                    }, DataViewCategoricalReader.prototype.getSeriesName = function(seriesIndex) {
                        if (this.hasAnyValidValues) return this.grouped[seriesIndex].name;
                    }, DataViewCategoricalReader.prototype.getSeriesDisplayName = function() {
                        if (this.hasAnyValidValues && this.dataHasDynamicSeries) return this.dataView.categorical.values.source.displayName;
            ...
            !function(data) {
                ...
                !function(DataViewConcatenateCategoricalColumns) {
                    function detectAndApply(dataView, objectDescriptors, applicableRoleMappings, projectionOrdering, projectionActiveItems) {
                        ...
                        if (dataViewCategorical) {
                            var concatenationSource = detectCategoricalRoleForHierarchicalGroup(dataViewCategorical, applicableRoleMappings), hierarchyVariationProperty = getHierarchyVariationProperty(dataView.metadata);
                    ...
                    function getHierarchyVariationProperty(metadata) {
                        var hierarchyColumn = _.find(metadata.columns, function(mc) {
                            return mc.expr instanceof data.SQHierarchyLevelExpr;
                        });
                        if (hierarchyColumn) {
                            var hierarchyLevelExpr = hierarchyColumn.expr, hierarchyExpr = hierarchyLevelExpr.arg;
                            if (hierarchyExpr instanceof data.SQHierarchyExpr) {
                                var variationSourceExpr = hierarchyExpr.arg;
                                if (variationSourceExpr instanceof data.SQPropertyVariationSourceExpr) return variationSourceExpr.property;
                            }
                        }
                    }
                    ...
                    function applyConcatenation(dataView, objectDescriptors, roleName, columnsSortedByProjectionOrdering, queryRefsToIgnore, hierarchyVariationProperty) {
                        ...
                        }), concatenatedColumnMetadata = createConcatenatedColumnMetadata(roleName, columnsSourceSortedByProjectionOrdering, queryRefsToIgnore, hierarchyVariationProperty), transformedDataView = inheritSingle(dataView);
                    ...
                    function concatenateValues(columnsSortedByProjectionOrdering, queryRefsToIgnore, formatStringPropId) {
                        for (var concatenatedValues = [], _i = 0, columnsSortedByProjectionOrdering_1 = columnsSortedByProjectionOrdering; _i < columnsSortedByProjectionOrdering_1.length; _i++) for (var categoryColumn = columnsSortedByProjectionOrdering_1[_i], formatString = valueFormatter.getFormatString(categoryColumn.source, formatStringPropId), i = 0, len = categoryColumn.values.length; i < len; i++) if (!_.contains(queryRefsToIgnore, categoryColumn.source.queryName)) {
                    ...
                    function createConcatenatedColumnMetadata(roleName, sourceColumnsSortedByProjectionOrdering, queryRefsToIgnore, hierarchyVariationProperty) {
                        ...
                        if (hierarchyVariationProperty) {
                            ...
                            _.contains(queryRefsToIgnore, columnSource.queryName) || (concatenatedDisplayName = null == concatenatedDisplayName ? columnSource.displayName : concatenatedDisplayName + " " + columnSource.displayName);
                        }
                        var newRoles = {};
                        newRoles[roleName] = !0;
            ...
            !function(data) {
                ...
                !function(DataViewSelfCrossJoin) {
                    ...
                    function createGroupedValues(category, categoryValues, categoryLength, valuesArray) {
                        for (var nullValuesArray = createNullValues(categoryLength), valuesArrayLen = valuesArray.length, seriesData = [], i = 0; i < categoryLength; i++) {
                            for (var seriesDataItem = [], j = 0; j < valuesArrayLen; j++) {
            ...
            !function(data) {
                ...
                !function(DataViewPivotCategoricalToPrimaryGroups) {
                    ...
                    function canPivotCategorical(binding, mapping) {
                        return !!isPivotableAxis(binding.Primary) && (!(!isPivotableAxis(binding.Secondary) || 1 !== binding.Secondary.Groupings[0].Projections.length) && (!binding.DataReduction || !binding.DataReduction.Primary && !binding.DataReduction.Secondary));
                    }
                ...
                !function(FieldExprPattern) {
                    ...
                    var FieldExprPatternAggregateVisitor = function() {
                        ...
                        return FieldExprPatternIsAggregationVisitor.prototype.visitColumn = function(column) {
                            ...
                            return !0;
                        }, FieldExprPatternIsAggregationVisitor.prototype.visitTransformOutputRoleRef = function(transformOutputRoleRef) {
                            return !1;
        ...
        !function(powerbi) {
            ...
            !function(DataViewAnalysis) {
                ...
                function validateRange(value, roleCondition, ignoreMin) {
                    if (roleCondition) return !ignoreMin && void 0 !== roleCondition.min && roleCondition.min > value ? DataViewMappingMatchErrorCode.conditionRangeTooSmall : void 0 !== roleCondition.max && roleCondition.max < value ? DataViewMappingMatchErrorCode.conditionRangeTooLarge : void 0;
                }
                ...
                function areAllPropertiesValid(requiredProperties, objectDescriptors, objectDefinitions) {
                    ...
                    return _.every(requiredProperties, function(requiredProperty) {
                        ...
                        var objectDefinitionValue = DataViewObjectDefinitions.getValue(objectDefinitions, requiredProperty, objectDefinitionProperty[0].selector);
                        return !(!objectDescriptorValue || !objectDefinitionValue) && powerbi.data.DataViewObjectEvaluator.evaluateProperty(staticEvalContext, objectDescriptorValue, objectDefinitionValue);
                    });
                }
            ...
            !function(data) {
                ...
                !function(DataViewRegression) {
                    ...
                    function getScalarKeyFromDataViewObjects(objects) {
                        return powerbi.DataViewObjects.getValue(objects, powerbi.visuals.cartesianChartProps.scalarKey.scalarKeyMin);
                    }
        ...
        !function(powerbi) {
            ...
            !function(ScriptResultUtil) {
                ...
                function extractScriptResultDefaultFromDataViewMappingScriptDefinition(scriptMapping) {
                    if (scriptMapping) {
                        var scriptSourceDefault = scriptMapping.scriptSourceDefault, scriptProviderDefault = scriptMapping.scriptProviderDefault;
                        return getScriptInfo(scriptSourceDefault, scriptProviderDefault);
                    }
                }
                function getScriptInfo(source, provider) {
                ...
                function getScriptInput(projections, selects, schema, customRoleSupport) {
                    var scriptInput = {
                ...
                !function(segmentation) {
                    ...
                    !function(DataViewMerger) {
                        ...
                        function mergeCategorical(source, segment) {
                            if (source.categories && segment.categories) for (var segmentCategoriesLength = segment.categories.length, categoryIndex = 0; categoryIndex < segmentCategoriesLength; categoryIndex++) {
                ...
                !function(EqualsToInRewriter) {
                    ...
                    var Rewriter = function(_super) {
                        ...
                        return InBuilder.prototype.add = function(operand, value) {
                            ...
                            if (!this.cancelled && this.operand) return data.SQExprBuilder.inExpr([ this.operand ], _.map(this.values, function(v) {
                                return [ v ];
                            }));
                        }, InBuilder;
                    }();
            ...
            !function(data) {
                ...
                !function(SQExprUtils) {
                    function supportsArithmetic(expr, schema) {
                        ...
                        return !(!metadata || !type) && (type.numeric || type.dateTime || type.duration);
                    }
                    ...
                    var SQExprDefaultNameGenerator = function(_super) {
                        ...
                        return __extends(IsDefaultValueVisitor, _super), IsDefaultValueVisitor.prototype.visitCompare = function(expr) {
                            return expr.comparison === data.QueryComparisonKind.Equal && expr.right.accept(this);
            ...
            !function(data) {
                var SemanticQueryRewriter = function() {
                    ...
                    return SemanticQueryRewriter.prototype.rewriteFrom = function(fromValue) {
                        for (var fromContents = {}, originalFrom = fromValue, originalFromKeys = originalFrom.keys(), i = 0, len = originalFromKeys.length; i < len; i++) {
            ...
            !function(data) {
                var ArrayExtensions = jsCommon.ArrayExtensions, SemanticQuery = function() {
                    ...
                    return SemanticQuery.create = function() {
                        ...
                        for (var unreferencedKeys = unreferencedKeyFinder.result(), i = 0, len = unreferencedKeys.length; i < len; i++) from.remove(unreferencedKeys[i]);
                        ...
                        for (var updatedWhere = [], from = this.fromValue.clone(), i = 0, len = values.length; i < len; i++) {
                        ...
                        for (var updatedWhere = this.where(), incomingWhere = filter.where(), from = this.fromValue.clone(), i = 0, len = incomingWhere.length; i < len; i++) {
                ...
                var SQExprRewriterWithSourceRenames = function(_super) {
                    ...
                    return __extends(SQExprRewriterWithSourceRenames, _super), SQExprRewriterWithSourceRenames.prototype.visitEntity = function(expr) {
                        ...
                        for (var updatedExprs, i = 0, len = exprs.length; i < len; i++) {
            ...
            !function(data) {
                ...
                function getExprValueFromTable(expr, selectTransforms, table, rowIdx) {
                    ...
                    if (!(_.isEmpty(rows) || rows.length <= rowIdx)) {
                        ...
                        if (!(selectIdx < 0)) for (var colIdx = 0, colLen = cols.length; colIdx < colLen; colIdx++) if (selectIdx === cols[colIdx].index) return rows[rowIdx][colIdx];
                    }
                }
                function findAggregateValue(expr, selectTransforms, columns) {
                    var selectIdx = findSelectIndex(expr.arg, selectTransforms);
                    if (!(selectIdx < 0)) for (var colIdx = 0, colLen = columns.length; colIdx < colLen; colIdx++) {
